        ...
        process.nextTick(function(){
            if ('open' == client.conn.readyState) {
                ...
                socket.on('puuid',function(data){
                    ...
                    socket.uuid= uuid;
                    var psocket;
                    if(self.psocketMap.hasOwnProperty(uuid)){
                        psocket = self.psocketMap[uuid];
                        psocket.add(socket);
                    }else{
                        psocket = new PSocket(this,socket);
                        self.sockets.push(psocket);
                        self.psocketMap[uuid]=psocket;
...
PNamespace.prototype.removePSocket = function(psocket){
    if(this.psocketMap.hasOwnProperty(psocket.uuid)){
        delete this.psocketMap[psocket.uuid];
    }
...
PNamespace.prototype.emit = function(ev){
    // TODO: either remove or modify after adapter
